...
class BaseTrainRunner:
    ...
    def __init__(self, **kwargs):
        ...
        if kwargs["is_continue"] and kwargs["timestamp"] == "latest":
            if os.path.exists(
                os.path.join("../", kwargs["exps_folder_name"], self.expname)
            ):
        ...
        utils.configure_logging(
            kwargs["debug"], kwargs["quiet"], os.path.join(self.log_dir, "log.txt")
        )
        ...
        for folder in ["training", "preprocess", "utils", "model", "datasets", "confs"]:
            ...
            os.system(
                """cp -r ./{0}/* "{1}" """.format(
                    folder, os.path.join(self.code_path, folder)
                )
            )

        os.system(
            """cp -r {0} "{1}" """.format(
                kwargs["conf"], os.path.join(self.code_path, "confs/runconf.conf")
            )
        )
        ...
        if self.conf.get_string("train.data_split") == "none":
            ...
            train_split_file = "./confs/splits/{0}".format(
                self.conf.get_string("train.data_split")
            )
        ...
        self.eval_dataloader = torch.utils.data.DataLoader(
            self.ds, batch_size=1, shuffle=True, num_workers=0, drop_last=True
        )
        ...
        self.network = utils.get_class(self.conf.get_string("train.network_class"))(
            conf=self.conf.get_config("network"), latent_size=self.latent_size
        )
        ...
        self.loss = utils.get_class(self.conf.get_string("network.loss.loss_type"))(
            **self.conf.get_config("network.loss.properties")
        )

        self.lr_schedules = BaseTrainRunner.get_learning_rate_schedules(
...
class StepLearningRateSchedule(LearningRateSchedule):
    ...
    def get_learning_rate(self, epoch):
        return np.maximum(
            self.initial * (self.factor ** (epoch // self.interval)), 1.0e-5
        )
